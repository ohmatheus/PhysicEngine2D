...
private:
	...
	virtual void Update(float frameTime) override
	{
		...
		const float InvMassAdd = 1 / mass + 1 / mass;
		const float epsilonPlusOne = 1.9f;
		...
		for (size_t i = 0; i < m_circles.size(); ++i)
		{
			for (size_t j = i + 1; j < m_circles.size(); ++j)
			{
				...
				{
					Vec2 colNormal = diffPos;
					colNormal.Normalize();

					Vec2 normalizedSpeed2 = c2->speed;
					normalizedSpeed2.Normalize();
					Vec2 normalizedSpeed1 = c1->speed;
					normalizedSpeed1.Normalize();

					float projectedSpeed2 = c2->speed.dot(colNormal);
					float projectedSpeed1 = c1->speed.dot(colNormal);

					Vec2 J = ((colNormal * projectedSpeed2 - colNormal * projectedSpeed1) * -epsilonPlusOne) / InvMassAdd;
					...
					float penetrationFactor = (2 * RADIUS - diffPos.GetLength()) * 0.5f;

					Vec2 c2Direction = colNormal * penetrationFactor;
					Vec2 c1Direction = colNormal * -penetrationFactor;

					c2->position += c2Direction;
					c1->position += c1Direction;
				}
			}
		}
